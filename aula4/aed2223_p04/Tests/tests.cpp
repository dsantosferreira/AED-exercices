...
TEST(test, extra_minPlatforms){
    EXPECT_EQ(1,FunSortProblem::minPlatforms({8.00, 8.20, 8.10, 8.30}, {8.09, 8.29, 8.19, 8.39}));
    EXPECT_EQ(4,FunSortProblem::minPlatforms({7.00, 7.10, 7.20, 7.30}, {8.00, 7.50, 7.45, 7.41}));
    EXPECT_EQ(2,FunSortProblem::minPlatforms({10.30, 10.00}, {11.00, 10.30}));
    EXPECT_EQ(3,FunSortProblem::minPlatforms({10.00, 10.10, 10.21, 10.31, 10.40, 5.00}, {10.10, 10.20, 10.22, 11.00, 11.00, 15.00}));
}
...
TEST(test, numInversions){
    ...
    EXPECT_EQ(0,FunSortProblem::numInversions({1,2,3,4,5,6}));
    EXPECT_EQ(15,FunSortProblem::numInversions({6,5,4,3,2,1}));
    EXPECT_EQ(12,FunSortProblem::numInversions({1,8,2,7,3,6,4,5}));
}
